...
export function readEncryptionConfig(
  ...
  const parsedIdentities = identities
    ? unique(
        identities,
        "age-identities",
        /^AGE-SECRET-KEY-1[AC-HJ-NP-Z02-9]+$/,
      )
    : [];
  if (!mode || mode === "none") {
    if (recipients) fail("age-recipients requires encryption=age.");
    return { mode: "none", recipients: [], identities: parsedIdentities };
  }
  if (mode !== "age") fail("encryption must be none or age.");
  return {
    mode: "age",
    recipients: recipients
      ? unique(recipients, "age-recipients", /^age1[ac-hj-np-z02-9]+$/)
      : [],
...
export async function decryptState(
  ...
  try {
    ...
    for (const identity of config.identities) decrypter.addIdentity(identity);
