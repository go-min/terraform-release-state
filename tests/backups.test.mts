...
test("backup retention distinguishes state assets from metadata", () => {
  ...
  assert.equal(
    isBackupAsset(
      "state.metadata.tfstate.backup-20260725T120000Z",
      "state.metadata.tfstate",
    ),
    true,
  );
  assert.equal(
    isBackupAsset("other.tfstate.backup-20260725T120000Z", "terraform.tfstate"),
    false,
  );
});
...
test("backup cleanup recognizes metadata format", () => {
  assert.deepEqual(
    metadataAssetNames("terraform.tfstate.backup-20260725T120000Z"),
    ["terraform.tfstate.backup-20260725T120000Z.metadata.json"],
  );
});
